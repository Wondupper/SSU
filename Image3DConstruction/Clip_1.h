...
bool clip(vec2& A, vec2& B, float minX, float minY, float maxX, float maxY)
{
	...
	{
		if (codeA & codeB)
		{   // поразрядное И не равно нулю
			return false; // отрезок полностью невидим
		}		if (codeA == 0)
		{   // если A видима, то B невидима
		...
		if (codeA & 1)
		{
			// точка A левее области видимости
